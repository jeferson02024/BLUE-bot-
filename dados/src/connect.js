...
try {
    ...
    DEBUG_MODE = config.debug === true || process.env.BLUE_DEBUG === '1';
    if (DEBUG_MODE) {
...
const codeMode = process.argv.includes('--code') || process.env.BLUE_CODE_MODE === '1';
...
async function createGroupMessage(BLUESock, groupMetadata, participants, settings, isWelcome = true) {
    ...
    if (settings.image) {
        ...
        if (participants.length === 1 && isWelcome) {
            profilePicUrl = await BLUESock.profilePictureUrl(participants[0], 'image').catch(() => profilePicUrl);
        }
...
async function handleGroupParticipantsUpdate(BLUESock, inf) {
    try {
        ...
        const botId = BLUESock.user.id.split(':')[0];
        ...
        let groupMetadata = await BLUESock.groupMetadata(from).catch(err => {
        ...
        switch (inf.action) {
            case 'add': {
                ...
                if (membersToRemove.length > 0) {
                    await BLUESock.groupParticipantsUpdate(from, membersToRemove, 'remove').catch(err => {
                        console.error(`❌ Erro ao remover membros do grupo ${from}: ${err.message}`);
                    });
                    
                    await BLUESock.sendMessage(from, {
                ...
                if (membersToWelcome.length > 0) {
                    const message = await createGroupMessage(BLUESock, groupMetadata, membersToWelcome, groupSettings.welcome || {
                        text: groupSettings.textbv
                    });
                    
                    await BLUESock.sendMessage(from, message).catch(err => {
            ...
            case 'remove': {
                if (groupSettings.exit?.enabled) {
                    const message = await createGroupMessage(BLUESock, groupMetadata, inf.participants, groupSettings.exit, false);
                    await BLUESock.sendMessage(from, message).catch(err => {
...
async function handleGroupJoinRequest(BLUESock, inf) {
    try {
        const from = inf.id;
        ...
        if (groupSettings.autoAcceptRequests && inf.action === 'created' && participantJid) {
            try {
                ...
                if (groupSettings.captchaEnabled) {
                    ...
                    await BLUESock.sendMessage(participantJid, {
                    ...
                    setTimeout(async () => {
                        ...
                        if (currentSettings.pendingCaptchas?.[participantJid]) {
                            ...
                            await BLUESock.groupRequestParticipantsUpdate(from, [participantJid], 'reject').catch(() => {});
                        }
                    }, 5 * 60 * 1000);
                } else {
                    // Auto-aceitar direto sem captcha
                    await BLUESock.groupRequestParticipantsUpdate(from, [participantJid], 'approve');
                }
            } catch (err) {
...
async function fetchLidWithRetry(BLUESock, jid, maxRetries = 3) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            const result = await BLUESock.onWhatsApp(jid);
...
async function fetchLidsInBatches(BLUESock, uniqueJids, batchSize = 5) {
    const lidResults = [];
    ...
    for (let i = 0; i < uniqueJids.length; i += batchSize) {
        ...
        const batchPromises = batch.map(jid => fetchLidWithRetry(BLUESock, jid));
...
async function updateOwnerLid(BLUESock) {
    const ownerJid = `${numerodono}@s.whatsapp.net`;
    try {
        const result = await fetchLidWithRetry(BLUESock, ownerJid);
        if (result) {
...
async function performMigration(BLUESock) {
    let scanResult;
    try {
    ...
    const { jidToLidMap, successfulFetches } = await fetchLidsInBatches(BLUESock, uniqueJids);
...
async function createBotSocket(authDir) {
    try {
        ...
        if (codeMode && !BLUESock.authState.creds.registered) {
            ...
            const code = await BLUESock.requestPairingCode(phoneNumber.replaceAll('+', '').replaceAll(' ', '').replaceAll('-', ''));
            console.log(`🔑 Código de pareamento: ${code}`);
            console.log('📲 Envie este código no WhatsApp para autenticar o bot.');
        }

        BLUESock.ev.on('creds.update', saveCreds);

        BLUESock.ev.on('groups.update', async (updates) => {
            ...
            const updatePromises = updates.map(async ([ev]) => {
                ...
                try {
                    const meta = await BLUESock.groupMetadata(ev.id).catch(() => null);
                    if (meta) {
        ...
        BLUESock.ev.on('group-participants.update', async (inf) => {
            if (DEBUG_MODE) {
            ...
            await handleGroupParticipantsUpdate(BLUESock, inf);
        });
        
        // Listener para solicitações de entrada em grupos (join requests)
        BLUESock.ev.on('group.join-request', async (inf) => {
            if (DEBUG_MODE) {
                console.log('\n🐛 ========== GROUP JOIN REQUEST ==========');
                console.log('📅 Timestamp:', new Date().toISOString());
            ...
            await handleGroupJoinRequest(BLUESock, inf);
        });
        ...
        const processMessage = async (info) => {
            ...
            if (typeof indexModule === 'function') {
                await indexModule(BLUESock, info, null, messagesCache, rentalExpirationManager);
            } else {
        ...
        const attachMessagesListener = () => {
            ...
            BLUESock.ev.on('messages.upsert', async (m) => {
        ...
        BLUESock.ev.on('connection.update', async (update) => {
            const {
                connection,
                lastDisconnect,
                qr
            } = update;
            if (qr && !BLUESock.authState.creds.registered && !codeMode) {
            ...
            if (connection === 'open') {
                ...
                await updateOwnerLid(BLUESock);
                await performMigration(BLUESock);
                
                rentalExpirationManager.nazu = BLUESock;
