...
class AutoRestarter {
    constructor() {
        ...
        this.pidFile = path.join(__dirname, '../../../BLUE.pid');
        ...
        try {
            ...
            const child = spawn(process.execPath, [...nodeArgs, ...args], {
                ...
                env: {
                    ...process.env,
                    BLUE_RESTARTED: 'true',
                    BLUE_RESTART_COUNT: this.restartCount.toString()
                }
            });
    ...
    async start() {
        try {
            ...
            if (process.env.BLUE_RESTARTED === 'true') {
                await this.logEvent('restart_success', {
                    previousRestartCount: process.env.BLUE_RESTART_COUNT || 'unknown'
                });
            }
        } catch (error) {
